...
int main(void) {
    ...
    while(1) {
        ...
        if (I2C1->SR1 & (1 << I2C_SR1_ADDR)) {
            ...
            // 5. Ожидание STOP-условия
            while (!(I2C1->SR1 & (1 << I2C_SR1_STOPF)));
            
            // 6. КОРРЕКТНАЯ очистка STOPF (без сброса PE!)
            I2C_ClearSTOPF(I2C1);
            
            // 7. Явное освобождение линии SDA
            GPIOB->BSRR = (1 << (I2C1_GPIOB7_SDA + 16)); // Сброс SDA
        }
        
        // 8. Обработка ошибки подтверждения
        if (I2C1->SR1 & (1 << I2C_SR1_AF)) {
            I2C1->SR1 &= ~(1 << I2C_SR1_AF);
            
            // Сброс только при ошибке
            I2C1->CR1 &= ~(1 << I2C_CR1_PE);
            I2C1->CR1 |= (1 << I2C_CR1_PE);
            I2C1->CR1 |= (1 << I2C_CR1_ACK);
            
            // Освобождение линии SDA
            GPIOB->BSRR = (1 << (I2C1_GPIOB7_SDA + 16));
        }
    }
}
...
void I2C_ClearSTOPF(I2C_TypeDef* I2Cx) {
    ...
    tmp = I2Cx->CR1;  // Шаг 2: Чтение CR1
    (void)tmp;
    I2Cx->CR1 |= (1 << I2C_CR1_ACK);  // Гарантируем включение ACK
}
