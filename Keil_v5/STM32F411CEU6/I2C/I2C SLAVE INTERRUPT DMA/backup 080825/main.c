...
#define I2C_SR1_AF            10    // Acknowledge failure flag

#define I2C_SR2_BUSY          1
#define I2C_SR2_TRA           2     // Direction flag (0: receiver, 1: transmitter)
#define I2C_SR2_MSL           0     // Master/slave flag
...
int main(void) {
    ...
    I2C1->CR1 |= (1 << I2C_CR1_ACK);  // Enable ACK

    while(1) {
        // 1. Ожидание адресации слейва
        if (I2C1->SR1 & (1 << I2C_SR1_ADDR)) {
            // 2. Очистка флага ADDR (стр. 592 RM0090)
            I2C_ClearADDR(I2C1);
            
            // 3. Проверка направления передачи
            if (I2C1->SR2 & (1 << I2C_SR2_TRA)) {
                // Режим передатчика (мастер читает)
                I2C1->DR = received_data; // Отправка данных
                
                // 4. Ожидание завершения передачи (стр. 590 RM0090)
                while (!(I2C1->SR1 & (1 << I2C_SR1_BTF)));
            } 
            else {
                // Режим приемника (мастер пишет)
                received_data = I2C_ReadByte(I2C1);
            }
            
            // 5. Ожидание STOP-условия (стр. 592 RM0090)
            while (!(I2C1->SR1 & (1 << I2C_SR1_STOPF)));
            
            // 6. Критически важная очистка STOPF (стр. 592 RM0090)
            volatile uint32_t tmp = I2C1->SR1;  // Шаг 1: Чтение SR1
            (void)tmp;
            I2C1->CR1 |= (1 << I2C_CR1_ACK);   // Шаг 2: Запись в CR1
            
            // 7. Явное освобождение шины (добавлено по рекомендации RM)
            I2C1->CR1 &= ~(1 << I2C_CR1_PE);   // Отключение I2C
            I2C1->CR1 |= (1 << I2C_CR1_PE);    // Повторное включение I2C
            I2C1->CR1 |= (1 << I2C_CR1_ACK);   // Повторное включение ACK
        }
        
        // 8. Обработка ошибки подтверждения (стр. 591 RM0090)
        if (I2C1->SR1 & (1 << I2C_SR1_AF)) {
            I2C1->SR1 &= ~(1 << I2C_SR1_AF);  // Очистка флага AF
...
void I2C_Init(I2C_TypeDef* I2Cx, void(*remap_func)(void), uint8_t mode, uint8_t slave_addr) {
    ...
    // Программный сброс (стр. 581 RM0090)
    I2Cx->CR1 |=  (1 << I2C_CR1_SWRST);
    I2Cx->CR1 &= ~(1 << I2C_CR1_SWRST);

    // Настройка таймингов для стандартного режима (100 кГц)
    I2Cx->CR2 = (16 << 0);   // Частота периферии 16 МГц
    I2Cx->CCR = (80 << 0);   // CCR = 80 для 100 кГц (стр. 585 RM0090)
    I2Cx->TRISE = 17;        // TRISE = 17 для 16 МГц (стр. 586 RM0090)

    // Настройка адреса слейва (стр. 583 RM0090)
    I2Cx->OAR1 = (slave_addr << 1) | (1 << 14); // 7-битный адрес
    
    // Включение I2C
    I2Cx->CR1 |= (1 << I2C_CR1_PE);
    I2Cx->CR1 |= (1 << I2C_CR1_ACK); // Включение ACK
}
...
// Properly clear STOPF flag
void I2C_ClearSTOPF(I2C_TypeDef* I2Cx) {
    I2Cx->CR1 &= ~(1 << I2C_CR1_PE); // Disable peripheral
    I2Cx->CR1 |= (1 << I2C_CR1_PE);  // Re-enable peripheral
    I2Cx->SR1 &= ~(1 << I2C_SR1_STOPF);
}
