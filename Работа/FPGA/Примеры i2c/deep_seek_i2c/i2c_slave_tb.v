...
// Assignments
assign sda = dut.sda_oe ? 1'bz : (sda_drive ? 1'bz : 1'b0);
...
initial begin
    $display("=========================================");
    $display("I2C Slave Testbench Started");
    $display("=========================================");
...
begin
    ...
    if (dut.current_state == dut.IDLE) begin
...
begin
    ...
    if (dut.register_file[reg_addr] === reg_data_in) begin
        $display("  PASS: Register write successful, data=0x%02X", dut.register_file[reg_addr]);
        test_pass = test_pass + 1;
    end else begin
        $display("  FAIL: Register write failed, expected=0x%02X, got=0x%02X", reg_data_in, dut.register_file[reg_addr]);
        test_fail = test_fail + 1;
    end
...
task test_i2c_write;
begin
    static logic [7:0] wr_test_addr = 8'h50;
    ...
    // Check if data was written (through external interface)
    if (dut.register_file[wr_test_addr] === wr_test_data1) begin
        $display("  PASS: Register write successful, data=0x%02X", dut.register_file[wr_test_addr]);
        test_pass = test_pass + 1;
    end else begin
        $display("  FAIL: Register write failed, expected=0x%02X, got=0x%02X", wr_test_data1, dut.register_file[wr_test_addr]);
        test_fail = test_fail + 1;
    end
    if (dut.register_file[wr_test_addr + 1] === wr_test_data2) begin
        $display("  PASS: Register write successful, data=0x%02X", dut.register_file[wr_test_addr + 1]);
        test_pass = test_pass + 1;
    end else begin
        $display("  FAIL: Register write failed, expected=0x%02X, got=0x%02X", wr_test_data2, dut.register_file[wr_test_addr + 1]);
        test_fail = test_fail + 1;
    end
    #100;
    
    // Note: In real test, you would read back through I2C
...
begin
    ...
    // First write data to read later
    reg_addr = 8'h30;
    reg_data_in = 8'hC3;
    reg_write = 1;
    #100;
...
always @(dut.current_state) begin
    $display("Slave state changed to: %s", dut.current_state.name);
end

// Slave address monitor
always @(posedge clk) begin
    if (dut.scl_falling && dut.current_state.name == "RECV_SLAVE_ADDR" && dut.bit_counter == 3'd7)
        $display("    Slave address is: 0x%02X", dut.shift_reg[6:0]);
end
